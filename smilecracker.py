...
from smile_gui import Ui_Main_window
#from PyQt4.QtGui import QApplication
from PyQt4.QtGui import QMainWindow
from smilecracker_config import *
...
class Main_window_ex(QMainWindow, Ui_Main_window):
    ...
    def slot_monitor(self):
        ...
        if self.check_options(self.periferica_opt) == 0:
            ...
            if status[0] != 0:
                ...
                service_status=commands.getstatusoutput('service network-manager start')
                if service_status[0]!=0:
                    self.output(service_status[1], service_status[0])
                else:
                    self.output("start network-manager service " , service_status[0])
        else:
            service_status=commands.getstatusoutput('service network-manager stop')
            if service_status[0]!=0:
                self.output(service_status[1], service_status[0])
            else:
                self.output("stop network-manager service " , service_status[0])
    ...
    def slot_rescan_networks(self):
        ...
        # Prepare the command
        scan_command = 'airodump-ng --output-format csv --write /tmp/smile-scan '
        ...
        output_raw = commands.getoutput('cat /tmp/smile-scan*.csv')
        ...
        commands.getstatusoutput('rm /tmp/smile-scan*')
...
def init_config_dir():
    ...
    if not os.path.exists(config_dir):
        os.mkdir(config_dir)
        #subprocess.getstatusoutput('zenity --info --window-icon=/usr/local/buc/icons/attenzione.png --title="Smile WiFi" --text="Hello and Thanks for using Smile Wifi Cracker this is the first run, and ~/.smile-wifi is now created."')
